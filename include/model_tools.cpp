...
void clean_vertex(tinyobj::attrib_t& attrib, tinyobj::shape_t& shape)
{
    ...
    attrib.texcoords.erase(std::remove_if(attrib.texcoords.begin(), attrib.texcoords.end(), [&](const tinyobj::real_t& idx) {
    ...
    for (size_t face_id = 0; face_id < shape.mesh.num_face_vertices.size(); face_id++) {
        ...
        Eigen::VectorXi key(6);
        key << idx0.vertex_index, idx1.vertex_index, idx2.vertex_index, idx0.normal_index, idx1.normal_index, idx2.normal_index;
    ...
    shape.mesh.indices.erase(std::remove_if(shape.mesh.indices.begin(), shape.mesh.indices.end(), [&](const tinyobj::index_t& idx) {
    ...
    for (int i_mesh = 0; i_mesh < v_attribs.size(); i_mesh += 1) {
        ...
        for (size_t k = 0; k < shapes.mesh.indices.size(); k += shapes.mesh.num_face_vertices[face_index++]) {
            ...
            for (int l = 0; l < v_per_f; l++) {
                ...
                // v0/t0/vn0
                fprintf(fp, " %d/%d/%d", ref.vertex_index + 1 + vertex_already_assigned, ref.texcoord_index + 1 + tex_already_assigned, ref.normal_index + 1+ vertex_already_assigned);
            }
            fprintf(fp, "\n");
        }
...
void split_obj(const std::string& file_dir, const std::string& file_name, const float resolution)
{
    ...
    std::tie(attrib, shapes, materials)=load_obj(file_dir+"/"+file_name, file_dir);
    ...
    cv::Mat img((int)((ymax - ymin)/ resolution)+1, (int)((xmax - xmin)/ resolution)+1, CV_32FC1,cv::Scalar(0.f));
    std::vector<std::vector<std::vector<std::pair<size_t, size_t>>>> records(img.rows,
        std::vector<std::vector<std::pair<size_t, size_t>>>(img.cols, std::vector<std::pair<size_t,size_t>>()));
	...
	for(int building_idx=0; building_idx<buildings.size(); building_idx++)
	{
        ...
        int x_center = *std::max_element(buildings[building_idx].xs.begin(), buildings[building_idx].xs.end())+(*std::min_element(buildings[building_idx].xs.begin(), buildings[building_idx].xs.end()));
        int y_center = *std::max_element(buildings[building_idx].ys.begin(), buildings[building_idx].ys.end())+(*std::min_element(buildings[building_idx].ys.begin(), buildings[building_idx].ys.end()));
        ...
        write_obj(file_dir + "/" + std::to_string(building_num) + ".obj", cur_attr, std::vector<tinyobj::shape_t>{cur_shape}, materials);
        building_num += 1;
	}
