...
const float MERGED_THRESHOLD = 20;
...
int main(int argc, char** argv){
	...
	std::vector<std::pair<Eigen::Vector3f, Eigen::Vector3f>> trajectory;
	{
		...
		//params.xy_angle = 3.15;
		//params.xy_angle = 4.72;
		params.xy_angle = 6.2;
		//params.xy_angle = 7;
		...
		for(auto& item_building:buildings)
		{
			...
			else
			{
				...
				float z_step = (item_building.bounding_box_3d.max().z() + params.z_up_bounds - params.z_down_bounds) / trajectory.size();
				for (auto& item : trajectory) {
					Eigen::Vector3f& position = item.first;
					position.z() = item_building.bounding_box_3d.max().z() + params.z_up_bounds - (&item - &*trajectory.begin()) * z_step;
		...
		for (auto& item : trajectory)
		{
			while (height_map.get_height(item.first.x(), item.first.y()) + params.view_distance > item.first.z()) {
				item.first[2] += 5;
			}
